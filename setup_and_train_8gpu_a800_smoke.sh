#!/bin/bash
# ============================================================
# Smoke-test selected A800 runs sequentially on one 8-GPU node.
#
# Runs the same experiments as setup_and_train_8gpu_a800_all.sh,
# but limits each experiment to 3 training steps.
#
# Order:
#   1. 7B full
#   2. 3B full
...
run_train() {
    ...
    local experiment_name="${model_label}_${method}_smoke_${RUN_TIMESTAMP}"
    local log_path="${REPO_ROOT}/${experiment_name}.log"

    echo "============================================"
    echo "  Launching smoke run: ${experiment_name}"
...
echo "[FINISH] All A800 smoke runs completed."
